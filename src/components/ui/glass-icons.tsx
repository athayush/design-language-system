...
const GlassIcons: React.FC<GlassIconsProps> = ({ items, className }) => {
  ...
  return (
    <div className={`grid gap-4 grid-cols-3 mx-auto  overflow-visible ${className || ''}`}>
      {items.map((item, index) => (
        <button
          key={index}
          type="button"
          aria-label={item.label}
          className={`relative bg-transparent mx-auto outline-none w-[4.5em] mt-8 h-[4.5em] [perspective:24em] [transform-style:preserve-3d] [-webkit-tap-highlight-color:transparent] group ${
            item.customClass || ''
          }`}
        >
          <span
          ...
          <span className="absolute top-full left-0 right-0 text-center whitespace-nowrap leading-[2] text-base ">
            {item.label}
          </span>
        </button>
      ))}
